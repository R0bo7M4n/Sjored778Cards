--c25894012
--Made by R0bo7M4n
--People's Despair - Meklord Emperor Wisel
local s,id=GetID()
...
function s.thfilter(c)
	return c:IsType(TYPE_SPELL+TYPE_TRAP) and c:ListsArchetype(SET_MEKLORD) and c:IsAbleToHand()
end
...
function s.synfilter(c,ec)
	return c:GetEquipTarget()==ec and c:IsOriginalType(TYPE_SYNCHRO)
end
function s.negcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not Duel.IsExistingMatchingCard(s.synfilter,tp,LOCATION_SZONE,0,1,nil,c) then return false end
