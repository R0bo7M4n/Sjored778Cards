--c25894011
--Made by R0bo7M4n
--Heaven's Despair - Meklord Emperor Skiel
local s,id=GetID()
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.Destroy(g,REASON_EFFECT)>0 then
		local dc=g:GetFirst() 		
		if Duel.IsExistingMatchingCard(s.thfilter,tp,LOCATION_GRAVE,0,1,nil,dc) then
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
			local sg=Duel.SelectMatchingCard(tp,s.thfilter,tp,LOCATION_GRAVE,0,1,1,nil,dc)
			if #sg>0 then
...
function s.dircon(e)
	return Duel.IsExistingMatchingCard(s.dirfilter,e:GetHandlerPlayer(),LOCATION_SZONE,0,1,nil,e:GetHandler())
end
